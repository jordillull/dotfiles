...
myKeys conf@(XConfig {XMonad.modMask = modMask}) = M.fromList $
    ...
    , ((modMask,               xK_g     ), runOrRaise
        "chromium-browser" (className =? "Chromium"))

    -- send a mail
    -- , ((modMask,               xK_x     ), emailPrompt mySP ["rupa@lrrr.us"])
 
    -- search prompt 
    , ((modMask, xK_x), SM.submap $ searchEngineMap $ promptSearch mySP)
    ...
    , ((modMask,               xK_F1 ), sendMessage NextLayout
    ...
    , ((modMask,            xK_ntilde), windows W.swapMaster)
